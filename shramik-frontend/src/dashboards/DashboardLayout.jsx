...
import {
  ...
  RotateCcw, Menu, AlertCircle, FileText,
  Zap, Droplets, Hammer, Paintbrush, Snowflake, Flame, BrickWall, LayoutGrid
} from 'lucide-react';
// Import the new logo
...
const SERVICES = [
  ...
  { id: 'AC Technician', icon: Snowflake,   label: 'AC Technician' },
  { id: 'Welder',        icon: Flame,       label: 'Welder' },
  { id: 'Mason',         icon: BrickWall,   label: 'Mason' },
  { id: 'Tiler',         icon: LayoutGrid,  label: 'Tiler' },
];
...
const BookingModal = ({ isOpen, onClose, tech, onBookingSubmit }) => {
...
const DashboardLayout = () => {
  ...
  // RESTORED: Uses exact URLSearchParams formatting just like your original code
  const executeSearch = useCallback(async () => {
    ...
    try {
      ...
      // Ensure we don't send empty strings to backend
      if (minPrice && minPrice.trim() !== '') params.append("minPrice", minPrice);
      if (maxPrice && maxPrice.trim() !== '') params.append("maxPrice", maxPrice);
      if (locality && locality.trim() !== '') params.append("locality", locality);

      const res = await api.get(`/api/technicians/nearby?${params.toString()}`);
  ...
  }, [gpsCoordinates, radius, selectedService, minPrice, maxPrice, locality]);

  const executeAvgPriceFetch = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/price-reports/average?${params.toString()}`);
          ...
          <div className="max-w-[1400px] mx-auto w-full flex-1 flex flex-col min-h-0 h-full">
                ...
                <aside className="w-full lg:w-80 shrink-0 space-y-6">
                  <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl">
                    ...
                    <div className="mb-6">
                      ...
                      <input type="range" min="1" max="5000" step="10" value={radius} onChange={e => setRadius(parseInt(e.target.value))} className="w-full h-1.5 bg-slate-800 rounded-full appearance-none cursor-pointer accent-indigo-500" />
                    </div>
                    ...
                    <div className="space-y-2.5 pt-4 border-t border-slate-800">
                      <button onClick={handleSearch} disabled={loadingTechs} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl transition flex items-center justify-center gap-2 shadow-lg">
                        {loadingTechs ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />} Search
                      </button>
                      <button onClick={resetFilters} className="w-full py-2.5 bg-slate-800/50 border border-slate-700 text-slate-300 hover:text-white font-semibold rounded-xl transition text-sm">
                        Reset Filters
                      </button>
                    </div>
                ...
                <div className="flex-1 min-w-0 space-y-6">
                  ...
                  {loadingTechs ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-24 bg-slate-900/20 border border-slate-800/50 rounded-3xl border-dashed text-center px-4">
                      ...
                      <button onClick={handleSearch} className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition border border-slate-700">
            ...
            {activeTab === 'bookings' && (
              <div className="space-y-6 w-full pb-10">
                {/* Aligned Toggle container to the left */}
                <div className="flex bg-slate-900/60 p-1.5 rounded-2xl border border-slate-800/80 w-full max-w-md">
                  {[
                ...
                {/* Using a proportionally scaled grid that starts from the left */}
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {bookingsTab === 'active' && (
                    activeBookings.length === 0 ? (
                      <div className="col-span-full"><EmptyBookingsState text="No active bookings." /></div>
                  ...
                  {bookingsTab === 'completed' && (
                    completedBookings.length === 0 ? (
                      <div className="col-span-full"><EmptyBookingsState text="No job history." /></div>
...
const EmptyBookingsState = ({ text }) => (
  <div className="flex flex-col items-center justify-center py-20 bg-slate-900/20 border border-slate-800/50 rounded-3xl border-dashed text-center w-full">
